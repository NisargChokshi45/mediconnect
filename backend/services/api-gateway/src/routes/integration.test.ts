...
describe('Gateway Integration Tests', () => {
  ...
  const testProxy = async (
    ...
    // Test 400 error
    serviceMock[serviceMethod].mockRejectedValue({
  ...
  describe('Auth Routes', () => {
    it('should proxy auth register/login', async () => {
      await testProxy(authServiceClient, 'post', 'post', '/api/auth/register', {
        email: 't@t.com',
      });
  ...
  describe('Appointment Routes', () => {
    it('should proxy appointment routes', async () => {
      ...
      await testProxy(
        ...
        'get',
        'get',
        '/api/appointments/patient/p1/upcoming'
      );
      // NOTE: appointment gateway uses PUT on the backend for status update
      await testProxy(appointmentServiceClient, 'put', 'patch', '/api/appointments/123/status', {
        status: 'COMPLETED',
      });
    });
  });
