...
export function errorHandler(err: Error, req: Request, res: Response, _next: NextFunction) {
  ...
  if (err.message === 'NOTE_NOT_FOUND') {
    return res.status(404).json({ success: false, error: { code: 'NOTE_NOT_FOUND', message: 'Clinical note not found' } });
  }

  if (err.message === 'UNAUTHORIZED_DOCTOR') {
    return res.status(403).json({ success: false, error: { code: 'UNAUTHORIZED_DOCTOR', message: 'Only the doctor who created this note can modify it' } });
  }
