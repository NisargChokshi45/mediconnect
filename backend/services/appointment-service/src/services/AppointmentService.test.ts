...
describe('AppointmentService', () => {
  ...
  describe('createAppointment', () => {
    ...
    it('should create an appointment successfully', async () => {
      mockedAxios.get.mockResolvedValue({ data: { success: true, data: { insurancePolicyNumber: 'POL123' } } });
      mockAppointmentRepository.create.mockResolvedValue({ id: 'a1', ...createData, scheduledAt: new Date(createData.scheduledAt) } as any);
      mockInsuranceService.verifyEligibility.mockResolvedValue({ verified: true, eligibilityStatus: 'ACTIVE' });
    ...
    it('should throw PATIENT_NOT_FOUND if patient verify fails', async () => {
      ...
      await expect(appointmentService.createAppointment(createData as any)).rejects.toThrow('PATIENT_NOT_FOUND');
    });
    ...
    it('should throw DOCTOR_NOT_FOUND if doctor verify fails', async () => {
      ...
      await expect(appointmentService.createAppointment(createData as any)).rejects.toThrow('DOCTOR_NOT_FOUND');
    });
  });
  ...
  describe('getAppointmentById', () => {
    ...
    it('should throw if not found', async () => {
      ...
      await expect(appointmentService.getAppointmentById('a1')).rejects.toThrow('APPOINTMENT_NOT_FOUND');
    });
  });
  ...
  describe('updateAppointmentStatus', () => {
    it('should update status', async () => {
        mockAppointmentRepository.findById.mockResolvedValue({ id: 'a1' } as any);
        mockAppointmentRepository.update.mockResolvedValue({ id: 'a1', status: AppointmentStatus.COMPLETED } as any);
        const result = await appointmentService.updateAppointmentStatus('a1', { status: AppointmentStatus.COMPLETED });
        expect(result.status).toBe(AppointmentStatus.COMPLETED);
    });

    it('should handle cancellation', async () => {
        mockAppointmentRepository.findById.mockResolvedValue({ id: 'a1' } as any);
        mockAppointmentRepository.update.mockResolvedValue({ id: 'a1', status: AppointmentStatus.CANCELLED } as any);
        await appointmentService.updateAppointmentStatus('a1', { status: AppointmentStatus.CANCELLED, cancellationReason: 'Ill' });
        expect(mockAppointmentRepository.update).toHaveBeenCalledWith('a1', expect.objectContaining({ status: AppointmentStatus.CANCELLED, cancellationReason: 'Ill' }));
    });
    
    it('should throw UPDATE_FAILED if update null', async () => {
        mockAppointmentRepository.findById.mockResolvedValue({ id: 'a1' } as any);
        mockAppointmentRepository.update.mockResolvedValue(null);
        await expect(appointmentService.updateAppointmentStatus('a1', { status: AppointmentStatus.COMPLETED })).rejects.toThrow('UPDATE_FAILED');
    });

    it('should throw APPOINTMENT_NOT_FOUND if appointment not exists', async () => {
        mockAppointmentRepository.findById.mockResolvedValue(null);
        await expect(appointmentService.updateAppointmentStatus('a1', { status: AppointmentStatus.COMPLETED })).rejects.toThrow('APPOINTMENT_NOT_FOUND');
    });
  });
  ...
  describe('deleteAppointment', () => {
      ...
      it('should throw APPOINTMENT_NOT_FOUND if appointment not exists', async () => {
          mockAppointmentRepository.findById.mockResolvedValue(null);
          await expect(appointmentService.deleteAppointment('1')).rejects.toThrow('APPOINTMENT_NOT_FOUND');
      });
  });

  describe('getInsuranceCircuitBreakerStats', () => {
      it('should return stats', () => {
          mockInsuranceService.getCircuitBreakerStats.mockReturnValue({ state: 'CLOSED', stats: {} } as any);
