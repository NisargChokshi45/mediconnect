...
describe('Appointment Routes', () => {
  ...
  describe('GET /appointments/:id', () => {
    it('should get appointment', async () => {
      (AppointmentService.prototype.getAppointmentById as jest.Mock).mockResolvedValue({ id: 'a1' });
  ...
  describe('PATCH /appointments/:id/status', () => {
    it('should update status', async () => {
      ...
      const response = await request(app).patch('/appointments/a1/status').send({ status: 'COMPLETED' });
      expect(response.status).toBe(200);
    });
  });
  ...
  describe('DELETE /appointments/:id', () => {
    ...
    it('should handle errors', async () => {
      (AppointmentService.prototype.deleteAppointment as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).delete('/appointments/a1');
  ...
  describe('Error Catch Blocks', () => {
    it('should handle POST error', async () => {
      (AppointmentService.prototype.createAppointment as jest.Mock).mockRejectedValue(new Error('fail'));
    ...
    it('should handle GET by id error', async () => {
      (AppointmentService.prototype.getAppointmentById as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).get('/appointments/a1');
      expect(response.status).toBe(500);
    });

    it('should handle GET by patient id error', async () => {
      (AppointmentService.prototype.getPatientAppointments as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).get('/appointments/patient/p1');
      expect(response.status).toBe(500);
    });

    it('should handle GET by doctor id error', async () => {
      (AppointmentService.prototype.getDoctorAppointments as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).get('/appointments/doctor/d1');
      expect(response.status).toBe(500);
    });

    it('should handle GET upcoming error', async () => {
      (AppointmentService.prototype.getUpcomingAppointments as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).get('/appointments/patient/p1/upcoming');
      expect(response.status).toBe(500);
    });

    it('should handle PATCH status error', async () => {
      (AppointmentService.prototype.updateAppointmentStatus as jest.Mock).mockRejectedValue(new Error('fail'));
      const response = await request(app).patch('/appointments/a1/status').send({ status: 'COMPLETED' });
      expect(response.status).toBe(500);
    });
  });
